...
- (BOOL)checkCameraAuthority {
    ...
    if (res) {
        NSString *title = @"未开启权限";
        NSString *message = @"是否前往设置去开启相机权限？";
        UIAlertController *alert = [XHQAlertManager showTitle:title message:message enterTitle:@"打开" cancelTitle:@"取消" enterAction:^{
            [self openSetting];
        } cancelAction:nil];
